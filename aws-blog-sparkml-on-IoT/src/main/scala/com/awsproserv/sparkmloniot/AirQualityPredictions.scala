...
object AirQualityPredictions extends Logging {
  ...
  def main(args: Array[String]) {
    ...
    val schemaString = "deviceid latitude longitude sensorreading datetime"
    ...
    spark.sql("CREATE TABLE tbl_airqualityPredictions(deviceid string,latitude string,longitude string,datetime string,sensorreading string,airqualityprediction string) STORED AS TEXTFILE")
    ...
    unionStreams.foreachRDD ((rdd: RDD[Array[Byte]], time: Time) => {
        ...
        val featuresDF = sensorreadingsDF.select($"deviceid",$"latitude",$"longitude",$"datetime",vectorize($"sensorreading").as("features"))
        ...
        val lblairqualityresultsDF = airqualityresultsDF.withColumn("AirQuality", getText($"prediction"))
        ...
        spark.sql("insert into table tbl_airqualityPredictions select deviceid,latitude,longitude,datetime,SensorReading,AirQuality from airqualityTempTable")
  ...
  val vectorize = udf( (s:Double) =>  Vectors.dense(s) )
